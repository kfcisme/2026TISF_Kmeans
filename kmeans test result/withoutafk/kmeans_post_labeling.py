...
# -*- coding: utf-8 -*-
"""
K-Means 後設語意標籤化（含 AFK 守門 + 探險/紅石樣本覆寫 + 強化防呆）
-------------------------------------------------------------------
- 支援在 label_config.yaml 設定：
  - afk_threshold：群中心 AFK 守門（未達門檻的群，AFK 軸直接打掉）
  - unknown_percentile / softmax_confidence_min / secondary_gap_threshold
  - axes：語意軸權重（如 Exploration / Redstone / AFK）
  - sample_override：樣本級覆寫（explorer / redstone by percentile）
- 強化防呆：
  - 自動移除 Unnamed:* 殘留欄
  - columns.json 欄名對齊（僅在合理時覆蓋）
  - centers ↔ features 欄位自動對齊（缺的補 0、多的丟棄）
  - assignments 叢集欄位自動偵測

輸入（--out 資料夾內）
- kmeans_features.xlsx（需有 features_scaled）
- kmeans_results.xlsx（需有 centers_scaled, assignments, 建議有 cluster_profile_unscaled）

輸出
- labeled_results.xlsx（assignments_labeled/cluster_labels/axis_scores/axis_legend）
"""
...
def maybe_apply_columns_json(df: pd.DataFrame, columns_json: Path) -> pd.DataFrame:
    ...
    if cur_n == tgt_n + 1:
        ...
        if looks_like_index:
            print("[INFO] 偵測疑似索引殘留第一欄，將移除後再套用 columns.json。")
            df = df.iloc[:, 1:].copy()
            if df.shape[1] == tgt_n:
                df.columns = target_cols
...
def main():
    ap = argparse.ArgumentParser(description="K-Means 後設語意標籤化（AFK 守門 + 樣本覆寫 + 防呆）")
    ...
    afk_threshold = float(cfg.get("afk_threshold", 0.0))  # ★ 群中心 AFK 守門（0 表示關閉）
    ...
    # 讀取未標準化群中心（若存在，用於 AFK 群中心守門）
    try:
        cluster_profile_unscaled = read_sheet_tolerant(res_xlsx, ["cluster_profile_unscaled", "profiles", "unscaled"])
        cluster_profile_unscaled = drop_unnamed(cluster_profile_unscaled)
        has_unscaled_profiles = True
    except Exception:
        cluster_profile_unscaled = None
        has_unscaled_profiles = False
        print("[WARN] 找不到 cluster_profile_unscaled，AFK 群中心守門將略過。")
    ...
    # ---- ★ AFK 群中心守門（使用 YAML 的 afk_threshold）----
    if afk_threshold > 0 and has_unscaled_profiles:
        if "afk_ratio" in cluster_profile_unscaled.columns:
            bad_clusters = cluster_profile_unscaled.index[
                cluster_profile_unscaled["afk_ratio"] < afk_threshold
            ].tolist()
            if bad_clusters:
                print(f"[INFO] AFK 群中心守門：以下群 afk_ratio<{afk_threshold}，禁用 AFK 標籤：{bad_clusters}")
                for cid in bad_clusters:
                    if cid in axis_scores_df.index and "AFK" in axis_scores_df.columns:
                        axis_scores_df.loc[cid, "AFK"] = -1e9  # 直接打掉 AFK 分數
        else:
            print("[WARN] cluster_profile_unscaled 缺少 'afk_ratio' 欄位，無法套用 afk_threshold。")
    ...
    # ---- 樣本級覆寫：Explorer / Redstone（依 YAML）----
    ...
    def apply_sample_override(label_series: pd.Series, feats: pd.DataFrame, spec: dict, new_label: str) -> pd.Series:
        """
        spec:
          by_percentile: { feature: pct, ... }   # 以該欄位分位數做門檻
          also_require_any: [f1, f2, ...]        # （可選）至少滿足其中一個輔助訊號（以各自的中位數為門檻）
        """
    ...
    ov_explorer = cfg_override.get("explorer")
    if ov_explorer:
        out_assign["label"] = apply_sample_override(out_assign["label"], features_scaled_df, ov_explorer, "Exploration")

    ov_redstone = cfg_override.get("redstone")
    if ov_redstone:
        out_assign["label"] = apply_sample_override(out_assign["label"], features_scaled_df, ov_redstone, "Redstone")
    ...
    if afk_threshold > 0:
        print(f"[INFO] 已啟用 AFK 群中心守門，afk_threshold = {afk_threshold}")
